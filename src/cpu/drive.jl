...
ϕ = Array{Complex{Float32}, 3}(undef, Nx, Ny, Nz) # Keeping operations in Float32 to keep array size low and speed high.

# Initializing ψ(x, y, z, 1) at t = 1 (as array indexing starts from 1 in Julia).
# Initializing to a Gaussian Normal Distribution.

gauss_3d(x, y, z) = ((64/π)^(3/4)*exp(-64*(x^2 + y^2 + z^2)/2) + 0im) |> Complex{Float32}
gauss_2d(x, y) = ((64/π)^(1/2)*exp(-64/2(x^2 + y^2)) + 0im) |> Complex{Float32}

for idx in CartesianIndices(ϕ[:, :, :])
    xx, yy, zz = idx.I
